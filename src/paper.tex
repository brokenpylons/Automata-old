...
\begin{enumerate}
\item Ustvarimo novo začetno stanje in ga povežemo s stanji, ki jih je mogoče direktno doseči iz začetnega stanja pri $\Automaton{s}$ in $\Automaton{t}$.
...
\begin{align*}
  \N{r} &= \Char{a} \Spc \Char{b} \Union \Char{a} \Spc \Char{b} \Spc \Char{c} \Union \Char{a} \Spc \Char{c} \Spc \Char{d}\\
        &= \Char{a} \Seq (\Char{b} \Union \Char{b} \Spc \Char{c} \Union \Char{c})\\
        &= \Char{a} \Seq (\Char{b} \Seq (\Null \Union \Char{c}) \Union \Char{c})
\end{align*}
...
\begin{center}
  \begin{tikzpicture}
    ...
    \node[state, accepting, right=of u1] (u2) {$(3,3,\Err)$};
    \draw (u0) edge[->] node[auto]{$\Char{a}$} (u1);
    \draw (u1) edge[->] node[auto]{$\Char{b}$} (u2);

    \node[state, accepting, above right=of u2] (u3) {$(\Err, 4, \Err)$};
    \draw (u2) edge[->] node[auto]{$\Char{c}$} (u3);

    \node[state, below right=of u1] (u4) {$(\Err,\Err,3)$};
    \node[state, accepting, right=of u4] (u5) {$(\Err,\Err,4)$};
...
\begin{enumerate}
  \item Končna stanja $\Automaton{s}$ povežemo s stanji, ki jih je mogoče direktno doseči iz začetnega stanja istega avtomata (vsakega z vsakim).
  \item Začetno stanje $\Automaton{\Kleene{s}}$ označimo kot končno.
\end{enumerate}
...
\begin{center}
  \begin{tikzpicture}
    ...
    \node[state, initial, label=above left:$\Automaton{\N{r}}$] (v0) {$(1,1)$};
    ...
    \node[state, accepting, above=2cm of v1] (s) {$(2, \Err)$};
